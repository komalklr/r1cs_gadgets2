...
fn example_gadget_test() {
    // (3 + 4) * (6 + 1) = (40 + 9)
    //use rand_core::SeedableRng;assert!(example_gadget_roundtrip_helper(5, 7, 35).is_ok());
    // (3 + 4) * (6 + 1) != (40 + 10)
    //assert!(example_gadget_roundtrip_helper(3, 4, 10).is_err());
    // use rand::{SeedableRng,Rng};
    // let mut rng:Rng = SeedableRng::seed_from_u64(65u64);
    // let r = Scalar::from(rng.gen::<f64>());
